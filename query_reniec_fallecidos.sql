...
-- SELECT TOP 10 * FROM BD_SIRIM.dbo.RimReniecFallecidos -- 409,920
...
SELECT TOP 10 * FROM #tmp_fallec_join_pas_by_dni p WHERE p.sEstadoActual = 'EXPEDIDO'  

*/

SELECT COUNT(1) FROM #tmp_fallec_join_pas_by_dni p WHERE p.sEstadoActual = 'EXPEDIDO' AND p.[¿Vencido?] = 'vigente'
SELECT COUNT(1) FROM #tmp_fallec_join_pas_by_dni p WHERE p.sEstadoActual = 'EXPEDIDO' AND p.[¿Vencido?] = 'vigente'
                     ...
                     CASE
                        ...
                        WHEN p.dFechaExpiracion != '1900-01-01 00:00:00.000' THEN (
                           CASE
                              ...
                              -- WHEN p.dFechaExpiracion >= '2024-08-13 00:00:00.000' THEN 'Vencido'
                              ELSE 'Vigente'
                           END
                        )
...
FROM (
   ...
   SELECT 
      *,
      [#] = ROW_NUMBER() OVER (PARTITION BY f.sNumDocIdentidad, f.sPasNumero ORDER BY f.sCoincidenciaPor ASC)
   FROM (

         SELECT * FROM #tmp_fallec_join_pas_by_dni f WHERE f.[¿Vencido?] != 'No registra pasaporte' -- Registran pas
         UNION ALL
         SELECT * FROM #tmp_fallec_join_pas_by_datos f WHERE f.[¿Vencido?] != 'No registra pasaporte' -- Registran pas
...
FROM (
   ...
   SELECT
      f.*,
      [##] = ROW_NUMBER() OVER (PARTITION BY f.sPasNumero ORDER BY f.dFechaEmision DESC)
   FROM (
   ...
   WHERE 
      f.[sEstadoActual] = 'EXPEDIDO'
      -- AND f.[¿Vencido?] = 'Vencido'
...
SELECT COUNT( DISTINCT f.sNumDocIdentidad) FROM BD_SIRIM.dbo.RimReniecFallecidos f
INNER JOIN sim.dbo.SimPasaporte p ON f.sNumDocIdentidad=p.sNumeroDoc
WHERE p.dFechaExpiracion>='20240813' 
