...
(
	SELECT
		...
		[Fecha Tramite] = CONVERT(DATE, st.dFechaHora),
		[Fecha Ultima Etapa] = (
											SELECT TOP 1 COALESCE(setn.dFechaHoraFin, setn.dFechaHoraAud) 
											FROM SimEtapaTramiteNac setn 
											WHERE 
												setn.sNumeroTramite = st.sNumeroTramite
												AND setn.sEstado = 'F'
												AND setn.bActivo = 1
											ORDER BY
												setn.nIdEtapaTramite DESC
							),
		/* [Fecha Impresión] = (
...
WHERE
	n.[Fecha Ultima Etapa] >= '2016-01-01 00:00:00.000'
