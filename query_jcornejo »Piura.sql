...
SELECT
   ...
   [sTipoControl] = (m.sIdModulo),
   [sNacionalidad] = (pn.sNacionalidad),
   [sSexo] = (per.sSexo),
   [sRangoEdad] = CASE 
                     ...
                     ELSE '90 a más'
               END,
   [sOrigenDestino] = (pm.sNombre),
   [dFechaControl] = CAST(mm.dFechaControl AS DATE),
   [sViaTransporte] = vt.sDescripcion,
   [nTotalCtrlMig] = COUNT(mm.sIdMovMigratorio)
...
WHERE 
   mm.bAnulado = 0
   AND mm.bTemporal = 0
   AND mm.dFechaControl BETWEEN '2022-01-01 00:00:00.000' AND '2024-11-17 23:59:59.999'
...
SELECT 
   DATEPART(MM, p.dFechaEmision),
   COUNT(1)
FROM SimPasaporte p
WHERE
   p.dFechaEmision BETWEEN '2024-01-01 00:00:00.000' AND '2024-10-17 23:59:59.999'
GROUP BY
   DATEPART(MM, p.dFechaEmision)