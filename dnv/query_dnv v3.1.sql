...
-- 4 Buscar alertas por datos:

-- 4.1
/*
	PAS XDD533972	| LOPEZ UNZUETA MERCEDES MARINA		| ESPAÑOLA		| 20-10-1949
	PAS 171755008	| GONZALEZ URRUTIA EDMUNDO				| VENEZOLANA	| 29-08-1949
	PAS TD006156	| LEDEZMA DIAZ ANTONIO JOSE			| VENEZOLANA	| 01-05-1955
	PAS DVR006309	| GONZALEZ OXFORD ANAMARIA				| VENEZOLANA	| 07-02-1968
	PAS PAV070208	| VIEIRA RODRIGUES ANDERSON GREGORY	| ESPAÑOLA		| 08-04-1991
	PAS 104868087	| SMOLANSKY UROSA DAVID					| VENEZOLANA	| 27-05-1985

*/

SELECT 

   [Doc InvalidaciÓn] = dnv.nIdDocInvalidacion,
   [Fecha Emision] = di.dFechaEmision,
   [Nombre] = dnv.sNombre,
   [Paterno] = dnv.sPaterno,
   [Materno] = dnv.sMaterno,
   [Nacionalidad] = dnv.sIdPaisNacionalidad,
   [Num Doc Invalida] = di.sNumDocInvalida,
   [Motivo] = mi.sDescripcion,
   [Tipo Alerta] = tt.sDescripcion,
   [Estado] = IIF(dnv.bActivo = 1, 'Inhabilitado', 'Habilitado'),
   [Dependencia] = d.sNombre,
   [Observaciones] = di.sObservaciones
...
WHERE
   ...
   -- AND tt.sDescripcion = 'ALERTA ES INFORMATIVA'
   -- AND di.sObservaciones LIKE '%documento%'
	-- AND mi.sIdMotivoInv IN ('PANU', 'PPNV')
	-- dnv.sIdDocumento = 'PAS'
	/* dnv.sNumDocIdentidad IN (
		'XDD533972',
		'171755008',
		'TD006156',
		'DVR006309',
	...
	-- dnv.sNombre LIKE '%AND%'
	dnv.sPaterno LIKE '%GON%'
	AND dnv.sMaterno LIKE '%URR%'
