...
WHERE
   -- p.Id = 57401540
   -- p.FirstName LIKE '%Vladimir Patricio%'
   -- p.FirstName LIKE '%Vladimir%'
   -- p.LastName = 'CAUCOTT QUIROGA'
   p.FirstName = 'CAUCOTTQUIROGA'
   -- AND p.LastName = 'QUIROGA'
   -- p.LastName = 'CAUCOTT QUIROGA'

SELECT TOP 10 * FROM PNR
SELECT TOP 10 * FROM CreditCard

SELECT TOP 10 * FROM Sesi

SELECT * 
FROM Document d
WHERE d.Number = 'F77127320'
...
SELECT COUNT(1) FROM Ph

SELECT TOP 10 * FROM MaritimeAddress
